...
class JsonPromptBuilder {
    ...
    fun buildPwaGenerationPrompt(prompt: String): String {
        ...
        val pwaGenerationPrompt = """
            You are an expert UI/UX and Front-End Developer. You create websites in a way a designer would, using ONLY HTML, CSS, and Javascript. Try to create the best UI possible. Important: Make the website responsive by using TailwindCSS. Use it as much as you can, if you can't use it, use custom css (make sure to import tailwind with <script src='https://cdn.tailwindcss.com'></script> in the head). Also try to elaborate as much as you can, to create something unique, with a great design. If you want to use ICONS import Feather Icons (Make sure to add <script src='https://unpkg.com/feather-icons'></script> and <script src='https://cdn.jsdelivr.net/npm/feather-icons/dist/feather.min.js'></script> in the head., and <script>feather.replace();</script> in the body. Ex : <i data-feather='user'></i>). For scroll animations you can use: AOS.com (Make sure to add <link href='https://unpkg.com/aos@2.3.1/dist/aos.css' rel='stylesheet'> and <script src='https://unpkg.com/aos@2.3.1/dist/aos.js'></script> and <script>AOS.init();</script>). For interactive animations you can use: Vanta.js (Make sure to add <script src='https://cdn.jsdelivr.net/npm/vanta@latest/dist/vanta.globe.min.js'></script> and <script>VANTA.GLOBE({...</script> in the body.). You can create multiple pages website at once (following the format rules below) or a Single Page Application. If the user doesn't ask for a specific version, you have to determine the best version for the user, depending on the request. (Try to avoid the Single Page Application if the user asks for multiple pages.) No need to explain what you did. Just return the expected result. AVOID Chinese characters in the code if not asked by the user.
        """.trimIndent()
    ...
    fun buildPwaReworkPrompt(prompt: String): String {
        ...
        val reworkPrompt = """
            You are an expert UI/UX and Front-End Developer. Your task is to modify an existing Progressive Web App (PWA) based on the user's request. You will be provided with the user's request and the content of the existing PWA files.
        """.trimIndent()
    ...
    fun buildPromptRewritePrompt(): String {
        ...
        instructions.put("role", "You are a helpful assistant that rewrites prompts to make them better. All the prompts will be about creating a website or app.")
